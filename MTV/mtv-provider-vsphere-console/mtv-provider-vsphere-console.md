# Create vSphere provider using the console

All the steps can be carried out using the console - even the creation and upload of the vddk image.

<br><br>**In main menu, select `Migration for Virtualization` and there the submenu `Providers`.**<br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-start.png)

<br><br>**From the supported list of migration sources, select VMware. Keep the project as openshift-mtv.**<br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-select.png)

<br><br>**Once selected , immediately the window for configuring it is displayed.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-1.png)

<br><br>**Fill in the config options:**

**- keep the project name as openshift-mtv**
**- keep the provider type as selected above - it will give the option to select another provider. This is misleading and should be not selected.**
**- provide a name for the resource**
**- select the endpoint type**
**- provide the URL to vCenter as in https://dns.name.of.vcenter.domain/sdk**

<br><br>**Creating the vddk image is available here from the console:** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-vddk.png)

<br><br>**The vddk image should be downloaded from [VMware](https://developer.broadcom.com/sdks/vmware-virtual-disk-development-kit-vddk/latest).**<br><br>

<br><br>**Upload the vddk tarball: through `Browse`select the locally (your workstation) stored tarball - once you hit the `Upload` button, the image will be created automatically and stored into the local OpenShift image registry of the cluster.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-vddk-upload.png)

<br><br>**Then it takes some tens seconds to build the image:** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-vddk-imagebuild.png)

<br><br>**Eventually, the image location will be presented in the VDDK image field. There is no need to upload another one into a custom location.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-vddk-complete.png)

<br><br>**At this point, all information should be green - this doesn't say that it's correct but only the expected format of the values is correct:** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-config-allbutCredentials.png)

<br><br>**Enter your login credentials for the vCenter / ESXi host and add the certificate - alternatively, select to skip the certificate.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-credentials.png)

<br><br>**Once all information is filled in, hit the `Create provider` button. It takes now a few seconds only and the `Ready` sign should be there.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-ready.png)

<br><br>**Further down in the overview for this new provider, the provider inventory should list the resources of the migration source:** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-ready-inventory.png)

<br><br>**Now, on the main provider list, this new provider should be listed and should have `Ready` as the status.** <br><br>
![UI view of OpenShift console to create UDNs](images/mtv-provider-vsphere-console-all.png)
